...
**Key figures:** All six waveform figures are complete. All are captured from `can_mac_pcs_fce_tb` (integrated testbench preferred over unit testbenches - submodule-specific signals are observed from the DUT hierarchy). `full_fd_frame.pdf` (`#fig:full_fd_frame`) is the integration overview: complete FD frame on both nodes, SSP pulses and `sync_applied` visible. `bs.pdf` (`#fig:bs`) shows dynamic stuffing (points A-C in `s_data`) and fixed stuffing (points D-E from `s_sbc` onward, REQ-017, REQ-019). `pcs.pdf` (`#fig:pcs`) shows TDC: count-up A→B (transceiver loopback delay, 19 TQ), countdown C→D (from ESI bit, SSP fires at D, TDC delay=2 signaled to MAC; REQ-025, REQ-026). `arb.pdf` (`#fig:arb`) shows arbitration loss: both nodes enter `s_arbitration` at A, DUT 1 loses at B (REQ-021). `error_flag.pdf` (`#fig:error_frame`) shows error frame escalation: SOF bit error at A (TEC=8), TEC=64 at first bit of second error flag at B (each error flag bit is also a bit error, bus held recessive), TEC=128 and `s_error_passive` at C (REQ-022, REQ-023, REQ-030). `bus_off.pdf` (`#fig:bus_off_recovery`) shows bus-off recovery: error passive at B (TEC=128), bus off at C (TEC=256), 128 idle condition strobes counted, `s_error_active` restored at D, DUT 2 acknowledges at E and F (REQ-031).
