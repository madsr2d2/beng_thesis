...
-- Requirements:  
--
-- Description:   Test bench utility package for the CAN/CAN-FD design.
...
package pk_can_tb is
  ...
  -- CRC calculation per ISO 11898-1: 6.6.4.4
  ...
  -- Bus stream reference model: generates expected CAN bus bit stream from LLC frame
...
package body pk_can_tb is
  ...
  procedure append(v : std_logic_vector; raw : inout std_logic_vector; raw_len : inOut natural) is
    variable v_slv : std_logic_vector(v'length - 1 downto 0) := v;
  begin
    for i in v_slv'length - 1 downto 0 loop
      raw(raw_len) := v_slv(i);
      raw_len      := raw_len + 1;
    end loop;
  ...
  begin
    ...
    -- CRC17 for < 16 data bytes, else CRC21.
    v_fixed_len := 0;
