...
\note{\footnotesize\begin{itemize}
		...
		\item  The \textbf{guidance fields} -- side, format, observability, method -- answer "how should this be verified?" They inform the testbench design but do not themselves constitute evidence. The \textbf{tracking fields} -- status, label, file -- are what actually close the loop. \texttt{status} records whether the requirement is verified, \texttt{label} names the exact TB procedure, and \texttt{file} points to the source. Every closed requirement has a direct link to its evidence.
		\item  The requirements are stored in a TOML file. Direct AI edits to large structured files are quite error-prone, so an MCP server was developed alongside the verification plan -- enabling \textbf{safe and consistent AI-assisted edits} throughout the project.
	\end{itemize}}
...
\end{frame}
\note{\footnotesize\begin{itemize}\setlength{\itemsep}{5pt}
		\item So here are the results of the verification effort --- \textbf{28 of 37 requirements closed}. All P1 MAC, PCS, and FCE requirements are verified. The nine open items fall into three categories.
...
\end{frame}
\note{\footnotesize\begin{itemize}\setlength{\itemsep}{5pt}
		\item These waveforms are the direct evidence for TDC and PCS verification --- this is what correct TDC behavior looks like on the simulator.
		\item \textbf{Top waveform:} A$\to$B shows the TDC counter counting up as the TX echo propagates back --- 19\,TQ loop delay. C$\to$D is the countdown from BRS to the reserved bit. At D the SSP fires at the correct offset for bit-error monitoring.
		\item \textbf{Bottom waveform:} A full CAN FD frame --- arbitration phase at nominal rate, then the faster data phase after BRS. Shows a complete TX/RX round-trip through both DUTs.
		\item \textbf{REQ-022:} This waveform is the direct verification evidence that TDC correctly positions SSP by measuring the transceiver loop delay on the reserved bit.
	\end{itemize}}
...
\end{frame}
\note{\footnotesize\begin{itemize}\setlength{\itemsep}{5pt}
		\item This waveform shows the full bus-off cycle driven by injected errors from \texttt{p\_test\_ctrl} in the integration testbench.
		\item \textbf{Waveform:} TEC increments on each injected error. It crosses 128 (node enters \textbf{error passive}), then 256 (node enters \textbf{bus off} and stops driving). After 128 idle sequences counted by \texttt{can\_pcs}, the node recovers to \textbf{error active}.
		\item \textbf{Recovery:} \texttt{can\_pcs} emits \texttt{idle\_condition} strobes on 11 consecutive recessive bits. FCE counts 128 of these --- ISO mandates this extended quiet period before re-joining. The 128 sequences prevent a faulty node from rapidly cycling in and out of bus off.
		\item \textbf{Integration evidence:} This sequence requires FCE, PCS, and MAC FSM all operating correctly together --- it cannot be produced by any unit testbench alone.
	\end{itemize}}
