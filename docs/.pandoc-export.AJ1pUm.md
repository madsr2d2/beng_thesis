...
![Dynamic and fixed bit stuffing in a CAN FD MAC frame. Three dynamic stuff bit insertions in the TX data stream (A, B, C). Seven fixed stuff bit insertions in the TX data stream between  D and E. Waveform from `can_mac_pcs_fce_tb`.](figures/waveforms/bs.pdf){#fig:bs width=100%}


![Dual bit rate switching and TDC measurement in a CAN FD MAC frame. The TDC measurement starts at the transmission of the dominant res bit edge (A) and stops when the echo returns at RX (B). At the transmission of the ESI bit (C) the meassured TDC dealy is counted down and the secondary sample point strobes starts when the count down terminates (D). TDC delay is the is the measured delay in how The MAC controls the initiation of the TDC delay measurement through `next_bit_is_res` and bit rate switching through `next_bit_is_brs` and `data_phase_stop`. Waveform from `can_mac_pcs_fce_tb`.](figures/waveforms/pcs.pdf){#fig:pcs width=100%}

![Arbitration loss in `can_mac_pcs_fce_tb`. Both nodes start transmitting (`is_transmitter` = `true`) at the same time (A). The DUT 1 node looses arbitration at B after transmitting recessive and sampling dominant and continues as receiver (`is_transmitter` = `false`). Waveform from `can_mac_pcs_fce_tb`.](figures/waveforms/arb.pdf){#fig:arb width=100%}

![Active error frame sequence in `can_mac_pcs_fce_tb` following a forced bit error in the data field. DUT 1 detects first bit error at A after driving the dominant SOF bit and sampling recessive. DUT 1 starts transmission of the second error flag at B and goes error passive at C. Waveform from `can_mac_pcs_fce_tb`](figures/waveforms/error_flag.pdf){#fig:error_frame width=100%}

![Bus-off recovery in `can_mac_pcs_fce_tb`. Node DUT 1 sends first active error flag at A. Goes error passive at B and bus off at C. At D, the FCE has counted 128 idle-condition strobes from the PCS and sets the MAC error active again. At E and F, DUT 2 acknowledges the first two transmitted frames after bus off recovery. Waveform from `can_mac_pcs_fce_tb`](figures/waveforms/bus_off.pdf){#fig:bus_off_recovery width=100%}
...
| REQ-015 | MAC | both | CB, CE, FB, FE | white_box | code_inspection, waveform_inspection | complete | p_fsm:~613, test_normal | can_mac_pcs_fce_tb |  |
| REQ-016 | MAC | transmitter | FB, FE | white_box | simulation, waveform_inspection | complete | test_normal | can_mac_pcs_fce_tb |  |
| REQ-017 | MAC | both | FB, FE | black_box | simulation | complete | p_sbc_checker | can_mac_bs_tb |  |
| REQ-018 | MAC | transmitter | CB, CE, FB, FE | white_box | code_inspection, waveform_inspection | complete | p_fsm:~768, p_fsm:~796, test_normal | can_mac_pcs_fce_tb |  |
| REQ-019 | MAC | both | CB, CE, FB, FE | white_box | simulation, coverage | complete | Input Coverage, Output Coverage | can_mac_bs_tb | All bins in cov_input and cov_output must be hit. |
| REQ-020 | MAC | both | CB, CE, FB, FE | white_box | simulation | complete | test_normal | can_mac_pcs_fce_tb |  |
| REQ-021 | system | transmitter | CB, CE, FB, FE | black_box | simulation | complete | test_lost_arb | can_mac_pcs_fce_tb |  |
...
| REQ-023 | MAC | both | CB, CE, FB, FE | white_box | code_inspection, waveform_inspection | complete | p_fsm:~283, p_fsm:~294, p_fsm:~357, test_bus_off | can_mac_pcs_fce_tb |  |
| REQ-024 | MAC | both | CB, CE, FB, FE | white_box | code_inspection | complete | p_fsm:~370 | can_mac_fsm |  |
| REQ-025 | PCS | both | CB, CE, FB, FE | white_box | simulation | complete | test_normal | can_pcs_tb |  |
| REQ-026 | PCS | transmitter | FB, FE | black_box | simulation | complete | p_check_tdc_delay | can_pcs_tb |  |
| REQ-027 | PCS | both | CB, CE, FB, FE | white_box | code_inspection, waveform_inspection | complete | p_can_pcs:~177, p_can_pcs:~228, test_normal | can_pcs_tb |  |
| REQ-028 | PCS | both | CB, CE, FB, FE | white_box | simulation, code_inspection | complete | p_can_pcs:~280, p_can_pcs:~379, test_bus_off | can_mac_pcs_fce_tb |  |
| REQ-029 | FCE | both | CB, CE, FB, FE | black_box | simulation | complete | test_reset, test_bus_off_recovery | can_fce_tb | Sub-claim 1: TEC/REC reset shall take effect within one clock cycle of the request. Sub-claim 2: verify that Normal_mode_response is asserted after reset completes. |
| REQ-030 | FCE | both | CB, CE, FB, FE | black_box | simulation | complete | test_rule_a, test_rule_b, test_rule_c, test_rule_c_exception, test_rule_d, test_rule_e, test_rule_f_tx, test_rule_f_rx, test_rule_g, test_rule_h | can_fce_tb | Rule h: the '>127 → 119-127' clamp path must be explicitly confirmed in simulation. |
| REQ-031 | FCE | both | CB, CE, FB, FE | black_box | simulation | complete | test_bus_off, test_bus_off_recovery | can_fce_tb |  |
| REQ-032 | MAC | both | FB, FE | white_box | code_inspection, waveform_inspection | complete | p_fsm:~636, p_fsm:~755, test_normal | can_mac_pcs_fce_tb |  |
...
| REQ-034 | MAC | both | CB, CE, FB, FE | black_box | simulation | complete | test_normal | can_mac_pcs_fce_tb |  |
| REQ-035 | system | both | CB, CE, FB, FE | black_box | simulation | complete | test_rule_c_exception; normal_test, test_bus_off (waveform) | can_fce_tb.vhd; can_llc_mac_pcs_fce_tb | Sub-claim 1: confirm no TX before 11 consecutive recessive bits are detected at the SP. Sub-claim 2: single-node scenario with no ACK injection; TEC must reach error-passive but not exceed 255. |
