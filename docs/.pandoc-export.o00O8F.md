...
| REQ-022 | 6.6.21.2 | P1 | **MAC error detection** `\newline`{=latex} 1) Bit error: any sent/monitored mismatch. Exceptions: recessive non-stuff bits during arbitration or ACK slot; dominant bits monitored while sending a passive error flag. `\newline`{=latex} 2) Stuff error: detected at the sixth consecutive identical bit in any dynamically-stuffed field. `\newline`{=latex} 3) Form error: any unexpected fixed-form bit value or wrong fixed stuff bit polarity (FB, FE). Exception: a dominant bit at the last EOF bit or last delimiter bit is not a form error. `\newline`{=latex} 4) CRC error: FCRC mismatch; FD frames additionally require SBC match and correct SBC parity. `\newline`{=latex} 5) ACK error: transmitter detects no dominant bit in the ACK slot. |
