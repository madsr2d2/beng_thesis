...
![`can_pcs` operation (ISO 11898-1 sec. 7.2-7.4). Four groups are shown. The per-TQ control path (blue, top-left) runs before the segment FSM each TQ: the prescaler fires a TQ boundary, `v_do_sync` detects a recessive-to-dominant edge, and `do_hard_sync` takes priority over the segment case statement when asserted. The bit timing segments (green, centre) cycle through `s_sync_seg` (1 TQ fixed), `s_prop_seg`, `s_phase_seg1`, and `s_phase_seg2`; soft synchronization extends `s_phase_seg1` or shortens `s_phase_seg2` by up to SJW when `v_do_sync` fires mid-segment. The bus drive/sample events (yellow, right) show the three IO moments: SSP samples `rx_i` one TQ before the SP (transmitting only, when `ssp_active`); the SP samples `rx_i` and switches to data-phase timing on a recessive BRS; and the bit boundary drives `tx_o` and signals the MAC. The TDC pipeline (purple, top) activates at the BRS/ESI bit boundary when `next_bit_is_res`: recessive TQs are counted until the TX-to-RX echo arrives (`delay_count_tq` = propagation delay), then at the ESI bit boundary a countdown begins and `ssp_active` is set when it expires.](figures/pcs_fsm_timeline.png){#fig:can-pcs width=100%}
