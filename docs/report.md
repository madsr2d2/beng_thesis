...
Charzinski's analysis explicitly benchmarked CAN against the contemporary alternatives VAN and SCP under the same channel model, finding CAN's residual error probability to be several orders of magnitude lower - a result he attributed directly to the combination of five concurrent detection mechanisms and the fault confinement escalation model [@charzinski1994]. The fault-confinement and multi-master properties that distinguished CAN from its contemporaries were deliberately preserved in CAN FD: Hartwich's design goal was to extend payload and bit rate while leaving the arbitration and fault-confinement architecture unchanged, specifically to retain the properties that made CAN suitable for safety-relevant distributed control networks [@hartwich2012].
