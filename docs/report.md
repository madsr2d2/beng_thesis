...
This thesis describes the design, implementation, and verification of a CAN (Controller Area Network) and CAN-FD (Flexible Data rate) protocol controller. The implementation targets high-reliability engine controller applications and complies with ISO 11898-1 [@iso11898_1]. Key features include support for the CB, CE, FB, and FE frame formats, dual bit rate switching with Transmitter Delay Compensation (TDC) for the FD data phase, and a modular architecture structured around the ISO 11898-1 sub-layer model: Logical Link Control (LLC), Media Access Control (MAC), Physical Coding Sublayer (PCS), and Fault Confinement Entity (FCE).
