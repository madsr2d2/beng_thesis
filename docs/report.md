...
A CAN Classic frame opens with a dominant SOF bit that triggers hard synchronization in all receiving nodes, followed by the arbitration, control, data, and CRC fields, an ACK slot, and a seven-bit EOF delimiter (see @fig:can-frame-structure). In the arbitration field, the RTR bit is dominant for data frames and recessive for remote frames. The IDE bit distinguishes base frames (dominant, 11-bit ID) from extended frames (recessive, 29-bit ID). In extended frames, a recessive SRR placeholder precedes the IDE bit. The ACK slot carries a dominant bit driven by every receiver that has validated the CRC - the transmitter reports an acknowledgment error if it receives no dominant ACK (REQ-038, REQ-014).
