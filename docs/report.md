...
The key asymmetry between CC and FD concerns the CRC data feed. For FD frames, dynamic stuff bits within the arbitration region are included in the CRC computation, while for CC frames they are excluded (REQ-013). Fixed stuff bits in the FD CRC region are also included. This dual data-feed requirement is a direct consequence of REQ-013 and has concrete consequences for the MAC implementation described in @sec:impl-can-mac-crc. The CRC field is terminated by a recessive CRC delimiter bit (REQ-018). A received frame with a CRC mismatch causes the detecting node to transmit an error flag.
