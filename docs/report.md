...
The verification framework is OSVVM (Open Source VHDL Verification Methodology) [@osvvm], a VHDL-native library used throughout all testbenches in this project. Three capabilities are central. First, clock and reset utilities (`WaitForClock`, `WaitForLevel`) express all stimulus timing in clock cycles rather than time literals, keeping testbenches independent of simulation time resolution. Second, `AffirmIf`-based assertion logging records each check against a named test procedure and produces a structured pass/fail summary at the end of each simulation run, making pass/fail results directly traceable to named procedures that correspond to requirement IDs. Third, the constrained-random stimulus framework (`CovBin`, `RandPkg`) generates stimulus that satisfies defined coverage bins, ensuring corner cases are exercised without hand-writing stimulus for every input combination - particularly useful for the CRC and bit stuffing unit testbenches where the input space is large.
...
The alternative - flat port lists with individual `std_logic` signals - was used in the existing controller, where the FSM entity has 20 individual ports for its various submodule interfaces. This approach becomes unwieldy as the number of inter-module signals grows: the new design has many inter-module signals across its interfaces, and bundling them into records reduces the port list from dozens of lines to six record-typed ports per FSM entity.
...
The 27/38 requirement closure rate warrants careful interpretation. The 11 open requirements fall into three categories with different implications for production confidence. Seven are LLC requirements deferred pending `can_llc` implementation - a known scope boundary, not a gap in the implemented protocol engine. Three P2 requirements (REQ-011 remote frames, REQ-036 data consistency, REQ-037 error signaling enable) are non-blocking for the core use case: a node that omits remote frame support and lacks a configurable error-signaling disable is still a fully correct CAN FD participant. REQ-035 (lone-node ACK exemption) is fully verified across two testbenches. Sub-claim 2 (lone node never bus-off) is confirmed by `test_rule_c_exception` in `can_fce_tb` via the ISO 8.1.4.2.c Exception 1 mechanism. Sub-claim 1 (bus re-integration before TX) is verified by waveform inspection of `normal_test` and `test_bus_off` in `can_llc_mac_pcs_fce_tb`.
...
4. **Error-type-specific simulation coverage (REQ-022).** The current testbench verifies bit-error detection through recessive injection at frame start. Sub-claims 2-5 (stuff, form, CRC, and ACK error detection) are covered by code inspection only. CRC error is the most accessible: flipping any bit in the RX DUT's bus view causes its locally computed CRC to diverge from the transmitted one, with the mismatch detected when the CRC field is reached. No frame-level timing is required, and this can be added as a targeted extension to `can_mac_pcs_fce_tb`. Stuff, form, and ACK error injection each require a frame-aware stimulus source - stuff error must target a stuff bit, form error must target a fixed-format field, and ACK error must target the ACK slot. Closing these three sub-claims requires either the Everllence-internal CAN FD reference model with targeted error injection capability, or white-box FSM state signals exposed from `can_mac_fsm` to time the injection correctly.
