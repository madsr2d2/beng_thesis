...
abstract: |
  ...
  A structured requirements framework of 37 requirements was derived from ISO 11898-1 normative clauses, each linked to its source clause, verification method, and testbench assertion. 28 of 37 are closed against passing testbenches or code inspection. The remaining nine fall into three categories: six LLC-layer requirements deferred pending LLC sub-layer implementation, two requirements not applicable or optional by design, and one requiring a frame-aware reference model for error-type-specific injection coverage.
