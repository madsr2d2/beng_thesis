...
![CAN frame formats (CB, CE, FB, FE) and the error and overload flags, with field widths annotated per ISO 11898-1. The bus waveform below each format uses fixed-field polarities for protocol-defined bits and hatched regions for variable-content fields.](figures/frame_format.png){#fig:can-frame-structure height=95%}
