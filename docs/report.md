...
Eleven requirements remain open. Seven are LLC requirements (REQ-001 through REQ-005, REQ-033, REQ-038) deferred pending implementation of `can_llc`. Three P2 requirements - REQ-011 (remote frames), REQ-036 (MAC data consistency), and REQ-037 (error signaling enable) - are deferred as non-blocking. REQ-035 (lone-node ACK exemption, P1) is in progress. It has two sub-claims: (1) a node completes bus re-integration before transmitting, and (2) a lone node receiving no ACK becomes error-passive but never bus-off. Sub-claim 2 is covered by `test_rule_c_exception` in `can_fce_tb` - the ISO 8.1.4.2.c Exception 1 mechanism already implemented via `passive_tx_ack_error_exempt_1` prevents bus-off. Sub-claim 1 (bus re-integration before TX) has not yet been verified by simulation (@sec:future-work).
...
The 27/38 requirement closure rate warrants careful interpretation. The 11 open requirements fall into three categories with different implications for production confidence. Seven are LLC requirements deferred pending `can_llc` implementation - a known scope boundary, not a gap in the implemented protocol engine. Three P2 requirements (REQ-011 remote frames, REQ-036 data consistency, REQ-037 error signaling enable) are non-blocking for the core use case: a node that omits remote frame support and lacks a configurable error-signaling disable is still a fully correct CAN FD participant. REQ-035 (lone-node ACK exemption) is in progress rather than a complete gap. Its two sub-claims separate cleanly: the ISO 8.1.4.2.c Exception 1 mechanism - already implemented via `passive_tx_ack_error_exempt_1` and verified in `can_fce_tb` - ensures that a lone node reaches error-passive but never bus-off, satisfying sub-claim 2. The remaining open item is sub-claim 1: confirming by simulation that the node completes bus re-integration before transmitting.
...
2. **REQ-035: bus re-integration before TX (sub-claim 1).** REQ-035 has two sub-claims. Sub-claim 2 (lone node never reaches bus-off) is already verified by `test_rule_c_exception` in `can_fce_tb` - the ISO 8.1.4.2.c Exception 1 mechanism prevents TEC increment for error-passive nodes with no ACK, keeping the node at error-passive indefinitely. Sub-claim 1 requires a simulation confirming that the node completes bus re-integration (11 consecutive recessive bits) before transmitting after power-on or bus-off recovery.
