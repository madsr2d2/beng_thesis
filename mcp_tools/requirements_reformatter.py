#!/usr/bin/env python3
"""
CAN Requirements Reformatter MCP Server

Manages requirements in the new ISO-aligned format (from the reformatting agent).
Handles querying, updating, and validating requirements in the new TOML structure.
...
Usage:
    python -m mcp_tools.requirements_reformatter
"""
...
# ── FastMCP Server ────────────────────────────────────────────────────────────
mcp = FastMCP("requirements-reformatter")


# ── Requirements Reformatter Manager ──────────────────────────────────────────


class RequirementsReformatter:
    """Manager for requirements in the new ISO-aligned format using tomlkit."""
    ...
    def __init__(self, toml_path: Path):
        ...
        logger.info(f"RequirementsReformatter ready: {self.toml_path}")
    ...
    def query(
        ...
        has_flags: Optional[str] = None,  # CSV string of flag names
        is_blank_label: Optional[bool] = None,
        is_blank_file: Optional[bool] = None,
    ) -> list[dict]:
        """Query requirements by shape, scope, layer, and flags.

        Args:
            shape: triggered, invariant, liveness, reachability
            scope: frame, node, bus
            layer: LLC, MAC, PCS, FCE
            has_flags: Comma-separated flags (e.g. "COMPOUND,EXTERNAL_DEP")
                       Returns requirements that have ANY of these flags
        ...
        for req in requirements:
            ...
            # Flag matching (has_flags checks for presence of any listed flag)
            if has_flags:
                required_flags = set(f.strip() for f in has_flags.split(","))
                req_flags = set(req.get("flags", []))
                if not req_flags & required_flags:  # No intersection
                    continue
    ...
    def update_requirement(self, req_id: str, field: str, value) -> dict:
        ...
        Args:
            ...
            field: Field name (shape, scope, layer, label, file, notes, etc.)
            value: New value (string, list, etc.)
        """
    ...
    def bulk_update(self, field: str, value, **filters) -> dict:
        ...
        Args:
            ...
            value: New value
            **filters: shape, scope, layer, has_flags, is_blank_label, is_blank_file
        """
        if field not in self.VALID_FIELD_UPDATES:
            raise ValueError(
                f"Invalid field '{field}'. Valid: {self.VALID_FIELD_UPDATES}"
...
def get_manager(toml_path: Optional[Path] = None):
    ...
    if toml_path is None:
        # Auto-detect: if this is the first call, look for any .toml in requirements/
        # Priority: llc_requirements_draft.toml > mac_requirements_draft.toml > requirements.toml
        for name in [
            "llc_requirements_draft.toml",
            "mac_requirements_draft.toml",
            "requirements.toml",
        ]:
    ...
    return RequirementsReformatter(toml_path)
...
@mcp.tool()
def query_requirements_reformatted(
    shape: Optional[str] = None,
    scope: Optional[str] = None,
    layer: Optional[str] = None,
    ...
    """Query requirements in the new ISO-aligned format.
...
@mcp.tool()
def get_requirement_reformatted(req_id: str, toml_path: Optional[str] = None) -> str:
...
@mcp.tool()
def update_requirement_reformatted(
    req_id: str,
    field: str,
    value: str,
    toml_path: Optional[str] = None,
...
@mcp.tool()
def get_statistics_reformatted(toml_path: Optional[str] = None) -> str:
...
def main() -> None:
    ...
    logger.info("Starting Requirements Reformatter MCP server")
