...
-- Standard   : VHDL-2008
--------------------------------------------------------------------------------
-- Description: Structural wrapper instantiating can_mac_tx and can_mac_rx with
--              separate PCS interfaces and a merged FCE interface. The FCE
--              output is formed by OR-ing all pulse/level fields from both
--              paths. This is safe because CAN is half-duplex: only one path
--              is actively processing a frame at any time.
--------------------------------------------------------------------------------
...
entity can_mac is
  port (
    ...
    -- Single FCE interface (shared between TX and RX)
