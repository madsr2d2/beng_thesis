      ...
      if (rst_i = '1' or fce_i.bus_off = '1') then
        ...
        if pcs_i.sample_point = '1' then
          ...
          if is_transmitter and v_in_arbitration_field
            ...
            -- disable fixed stuffing, enter s_error_overload. The error-
            -- frame status (transfer_status, was_previous_frame_tx) is
            -- handled by s_error_overload itself.
            ...
            when s_eof =>
              if is_transmitter then
                if (bit_count = 0 and not ack_success_seen) then
                  ...
                  -- flag end with passive-TX exemption (ISO 6.6.20).
                  ack_error_caused_flag      <= true;
                  pcs_o.tx_data              <= not fce_i.error_active;
                  pcs_o.data_phase_stop      <= '1';
                  bs_o.fixed_bit_stuffing_en <= '0';
            ...
            -----------------------------------------------------------------
            -- s_error_overload: 6 flag bits (dominant for active errors and
            -- overloads, recessive for passive). TX tracks passive-flag
            -- exemption; RX uses a combined 14-bit flag+delim sequence.
            -----------------------------------------------------------------
            when s_error_overload =>
              if is_transmitter then
                ...
                -- RX: count 6 flag bits (driven by held tx_data from
                -- error entry), then advance to s_error_delimiter.
                if bit_count < c_error_flag_width - 1 then
                  bit_count <= bit_count + 1;
                else
                  bit_count     <= 0;
            ...
            -----------------------------------------------------------------
            -- s_error_delimiter: 8 recessive bits. Shared by TX and RX. TX
            -- additionally tracks primary-error and delimiter-too-late for
            -- the FCE; RX just counts.
            -----------------------------------------------------------------
            when s_error_delimiter =>
              if pcs_i.rx_data = c_dominant then
                -- Dominant restarts the error sequence.
                v_tx_polarity := c_dominant;
                v_drive_bit   := true;
                ...
                if is_transmitter then
                  if dominant_run_count = c_error_delimiter_width - 1 then
                    ...
                    if not overload and dominant_run_count = 0 then
