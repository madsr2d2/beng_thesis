...
--                2026-04-03  TMYAES:   [TRIT-4355] [FPGA] Controlling FSM form MAC layer in CAN-FD module
--
--------------------------------------------------------------------------------------------------------------------------------------------------------------
...
begin
  ...
  p_fsm : process(clk_i) is
    ...
    variable v_in_dynamic_stuff_bit_field : boolean;                            -- In dynamic stuff bit guard
    variable v_in_fixed_stuff_bit_field   : boolean;                            -- In fixed stuff bit guard
    variable v_not_stuff_bit              : boolean;                            -- Real bit received flag (not stuff bit)
  ...
  begin
    if rising_edge(clk_i) then
      if (rst_i = '1' or fce_i.bus_off = '1') then
        ...
        crc_rst                    <= '0';
        fce_o                      <= c_mac_to_fce_if_reset;
        pcs_o.do_hard_sync         <= '0';                                      -- Default: resynchronization (ISO 7.3.5.1 rule d)
        ...
        -- Evaluate guards
        -----------------------------------------------------------------
        v_in_fixed_stuff_bit_field   := (fsm_state = s_sbc) or (fsm_state = s_crc);
        v_in_dynamic_stuff_bit_field := (fsm_state = s_id) or (fsm_state = s_rtr_srr_rrs) or (fsm_state = s_ide) or (fsm_state = s_fdf_r1_r0) or (fsm_state = s_res_r0) or (fsm_state = s_brs) or (fsm_state = s_esi) or (fsm_state = s_dlc) or (fsm_state = s_data);
        ...
        -----------------------------------------------------------------
        if (pcs_i.sample_point = '1') and (v_in_dynamic_stuff_bit_field or v_in_fixed_stuff_bit_field) then
          -- Feed BS
          bs_o.valid <= '1';
          bs_o.data  <= pcs_i.rx_data;
          if (bs_i.valid = '1') then
            -- Stuff bit: feed FD CRC in arb region (ISO 11898-1: 6.6.4.4), check polarity
            if (v_in_dynamic_stuff_bit_field) then
              crc_o.valid_fd <= '1';
            ...
            v_not_stuff_bit := true;                                            -- Set the v_real_bit flag for the FSM logic
        ...
        case fsm_state is
          ...
          when s_bus_reintegration =>
            pcs_o.do_hard_sync <= '1';                                          -- ISO 7.3.5.1 rule c: hard sync during bus integration
          ...
          when s_ide =>
            if (v_not_stuff_bit) then
              llc_frame(c_conf_0_offset)(c_llc_frame_ide) <= pcs_i.rx_data;     -- Set IDE bit
              if (pcs_i.rx_data = c_recessive) then
          ...
          when s_fdf_r1_r0 =>
            if (v_not_stuff_bit) then
              llc_frame(c_conf_0_offset)(c_llc_frame_fdf) <= pcs_i.rx_data;     -- Set FDF bit
          ...
          when s_res_r0 =>
            ...
            if (v_not_stuff_bit) then
              -- Form error check: reserved bit(s) must be  dominant
              if pcs_i.rx_data = c_recessive then
                fce_o.error <= '1';
                bit_count   <= 0;
                fsm_state   <= s_error_overload;
